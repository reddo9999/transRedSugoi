...
export abstract class TranslationEngineWrapper implements TranslationEngineWrapperCache {
	...
	public abstract doTranslate (toTranslate: Array<string>, options : TranslatorEngineOptions): Promise<Array<string>>;

	public translate (rows: Array<string>, translationOptions: Partial<TranslatorEngineOptions>) {
        ...
        if (this.optionUseCache.getValue()) {
            ...
            if (toTranslate.length !== translations.length) {
                this.log(`[RedTranslator] Skipped ${translations.length - toTranslate.length} sentences due to cache hits.`);
            }
        }
        /**
        ...
        this.log(`[RedTranslator] Sending ${toTranslate.length} lines (processed from ${rows.length} rows) to ${this.getEngine().name}.`);
		...
		this.doTranslate(toTranslate, options)
            ...
            .finally(() => {
                ...
                this.log(`${pad}Performance: ${Math.round(10 * result.sourceText.length / translatorPerformance.end().getSeconds())/10} characters per second. ${Math.round(10 * rows.length / batchPerformance.getSeconds())/10} rows per second.`);
                options.always();
            });
	}
	...
	constructor(processorClass : typeof TextProcessor, thisAddon: Addon, extraOptions: Partial<TranslationEngineOptions>) {
	...
	public init() {
        ...
        options.form.push({
            ...
            "items": [
                {
                  "type": "button",
                  "title": "Reset settings to their default values (click then close options to apply)",
                  "onClick" : (evt : any) => {
                      this.storedOptions.forEach(option => {
                          try {
    ...
    public getProcessingOrder () {
        ...
        let orderStringArray = this.optionProcessingOrder.getValue().toLowerCase().replaceAll(/[\.,;\/\|\\]/g, ",").split(",");
    ...
    public optionMaxRequestLength = new TranslationEngineOption<number>({
        ...
        description : "The amount of characters that each batch will have under batch translations. Higher numbers are faster, but smaller numbers are safer - specially if saving project between each batch. Recommended value is what you should get translated in 10-30 secs, for minimal loss of work in case of errors/failure.",
        default: 5000,
        priority : 1,
        wrapper : this
    });
	...
	public optionProtectedPatterns = new TranslationEngineOption<string>({
		...
		default: [
            "// SG Quest Information group",
            /(<SGカテゴリ:.*>)|(<SGピクチャ:.*>)|(<SG[^:]+>)|(<SG.+?:)|(>)|(<SAC.+?:)/g.toString() + ",",
            "// Value reference",
            /[\\]*[_\-a-z]+\[.+?\]/gi.toString() + ",",
        ].join('\n'),
		description:
	...
	public optionSplittingPatterns = new TranslationEngineOption<string>({
		...
		default: [].join('\n'),
		description:    'Splits sentences according to patterns.' +
	...
	public optionBreakPatterns = new TranslationEngineOption<string>({
		...
		description:    'The core of Red Text Processor. Separates sentences according to when it looks like the previous sentence ended, sending them to the translator separatedly.\n' +
	...
	public optionIsolatePatterns = new TranslationEngineOption<string>({
		...
		description:    'This finds text inside brackets and translates it separatedly.\n' +
	...
	public optionSplitEndsPatterns = new TranslationEngineOption<string>({
		...
		description:    'These are to be used to match against text at either the start or the end of sentences. It works kind of like splitting - the matched symbol will be removed from the sentence before translation and added back later.\n' +
