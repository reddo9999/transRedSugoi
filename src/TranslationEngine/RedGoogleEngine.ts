...
export class RedGoogleEngine	extends TranslationEngineWrapper {
        ...
        let startThread = () => {
            ...
            if (batch.length == 0) {
                ...
                let sendToGoogle = () => {
                    ...
                    (<(...any:Array<any>) => Promise<any>> common.fetch)(this.optionUrl.getValue(), {
                        method		: 'get',
                        data: ({
                            client : "gtx",
                            sl: options.sl,
	...
	public optionUrl : TranslationEngineOption<string> =
	...
	public optionInnerDelay: TranslationEngineOption<number> =
		new TranslationEngineOption<number>({
			...
			id: 'innerDelay',
			default: 6000,
			description: 'The amount of time (in milliseconds) to wait between requests. The longer this is, the less likely you are to be blocked by Google. Numbers lower than 6 seconds tend to cause a ban after just a few translations.',
			name: 'Inner Delay',
            category: TranslationEngineOptionCategories.LIMITS,
            priority: -8,
		});
    ...
    new TranslationEngineOption<string>({
        ...
        default: "<tr>",
        description: 'Google only accepts a single input, but we\'re translating many texts. So we have to mark when one text ends and another begins. It doesn\'t matter what this value is, so long as it is something Google will not touch or move. Google likes HTML tags.',
        name: 'Line Delimiter',
