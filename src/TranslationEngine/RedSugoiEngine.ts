...
import { TranslationEngineOptionCategories, TranslationEngineWrapper } from './TranslationEngineWrapper';
...
export class RedSugoiEngine	extends TranslationEngineWrapper {
	...
	public doTranslate(toTranslate: string[], options : TranslatorEngineOptions): Promise<string[]> {
        this.resetStatus();
		let returnTranslations: (translations: Array<string>) => void;
		let returnError: (reason: string) => void;

        if (options.sl != "ja") {
            this.error(`[RedSugoi] The project specifies the source language as not being Japanese (${options.sl}). Since Sugoi Translator only supports Japanese as source, we will use Japanese instead.`);
        }

        if (options.tl != "en") {
            this.error(`[RedSugoi] The project specifies the destination language as not being English (${options.tl}). Since Sugoi Translator only supports English as destination, we will use English instead.`);
        }
        ...
        let startThread = () => {
            ...
            while (translatingIndex < toTranslate.length) {
                ...
                if (toTranslate[index] !== undefined && (batchLength == 0 || (batchLength + toTranslate[index].length) <= maximumBatchSize)) {
            ...
            if (batch.length == 0) {
                ...
                .catch((error) => {
                    ...
                    console.error("[REDSUGOI] ERROR ON FETCH USING " + myServer, "   Payload: " + batch.join("\n"), error);
                    this.error(`[RedSugoi] Error while fetching from ${myServer.url} - ${error.name}: ${error.message}\n${' '.repeat(11)}If all fetch attempts fail on this server, check if it's still up.`);
                })
                .finally(() => {
    ...
    public optionRemoveBreaks: TranslationEngineOption<boolean> = new TranslationEngineOption<boolean>({
        wrapper: this,
        id: 'removeBreaks',
        default: false,
        description: [
	...
	public optionConcurrency: TranslationEngineOption<number> =
		new TranslationEngineOption<number>({
			...
			default: 2,
			description: 'Maximum amount of requests sent to the servers at the same time. The best number if the one that results in no downtime for the servers.',
